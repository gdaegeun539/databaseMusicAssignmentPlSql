-- 음악 정보 뷰
create view music_view (musicid, musicname, playtime, singer, writer, composer, albumname, publishdate, genrename)
...
where mu.musicid = mk.musicid and mu.albumid = a.albumid and mu.genreid = g.genreid
with read only;
...
-- 이용자 회원가입
insert into userinfo values (trunc(dbms_random.value(1,1000000)), '입력값', '입력값','입력값','입력값', '0000-00-00', '입력값','KOR');
...
select u.usernum, trunc(dbms_random.value(1, 10000000)), 클라이언트입력값, 0, 현재날짜값
from userinfo u
where u.usernum = 현재유저변수값;
-- (공개여부 변경시)
update playlist
set is_public = 1
where p.plid = 현재재생목록값(클라이언트) and exists(select usernum
                                                    from userinfo
                                                    where usernum = 현재유저변수값);
-- 곡 추가[미완/정상작동보장x]
...
-- 고객지원
insert into support
select trunc(dbms_random.value(1,1000000)), u.usernum, st.inquiry_code, 현재시간, 0
...
-- 계정탈퇴요청
insert into support
select trunc(dbms_random.value(1,1000000)), u.usernum, 2, 현재시간, 0 -- 코드2번 계정삭제
from userinfo u
where u.usernum = 현재유저변수값;
-- 계정탈퇴처리
...
-- 앨범추가
declare
rand_album_id int := trunc(dbms_random.value(1,1000));
begin
insert into album
...
-- 음원추가
declare
rand_music_id int := trunc(dbms_random.value(1,10000));
begin
insert into music
